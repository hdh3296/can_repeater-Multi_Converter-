...
*/
		...
		if( (BefCan1RxSid != Can1RxSid) || ( (BefCan1RxEid & 0xffffff00) != (Can1RxEid & 0xffffff00) )){
			bCan1RxAll=0;
			return(0);
		} 
		...
		if(bCan1RxAll){
			...
			for(j=0;j<16;j++){
				RcvBuf[hostnm+j]=Can1RxBuf[j];
			}

			RcvBuf[hostnm + ELEV_TMP_STATUS]=Can1RxBuf[21];
			RcvBuf[hostnm + ELEV_CRT_KEY]   =Can1RxBuf[SL_mCrtExtMoveFlr];

			ThisHostReceive=AckHost;
			HomeCallButtonReceiveChk(AckHost);
		}
	}
	else{
        bCan1RxAll=0;
...
RUN_LED= !RUN_LED;
	    ...
	    if( (!C1TX0CONbits.TXREQ) && (Can1PollingTimer > 10) ){
			...
			if((Can1TxSid == Can1RxSid) && ((Can1TxEid & 0xfc000) == (Can1RxEid & 0xfc000))){
				if(Can1RxBuf[SL_mCrtExtMoveFlr] > 0){
					Can1RxTimer=0;	
					return(0);
				}
			}	
			...
			if( !Can1TxData(0,0)){
				Can1PollingTimer=0;
    			while( (C1TX0CONbits.TXREQ) && (Can1PollingTimer<12) );
				Can1PollingTimer=12;
				Can1HostReqCmdLoad();
				if( !Can1TxData(0,0)){						
					return(1);
