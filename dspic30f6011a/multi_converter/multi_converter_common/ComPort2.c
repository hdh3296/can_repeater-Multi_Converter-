...
void  __attribute__((section(".usercode")))   Com2Init(unsigned int baudrate_val)
{
	TRISFbits.TRISF4=1;  //uart rx
	TRISFbits.TRISF5=0;  //uart tx


    U2BRG=(unsigned int)baudrate_val;			
...
void      __attribute__((section(".usercode"))) Com2TxStart(void)
{
   	...
   	Com2RxStatus=TX_SET;
	TXEN_485=1;
	U2TXREG=Com2TxBuffer[Com2TxThisPt];
   	Com2TxThisPt++;   
	TXEN485Timer=0;
}
